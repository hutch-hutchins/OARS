...
pub fn step(word: u32, pc: u32, regs: &mut RegisterFile) -> Result<u32, OarsError> {
    ...
    let v = match f::funct3(word) {
        ...
        0x4 => {                                                             // DIV (signed)
            if b == 0 { u32::MAX }
            else if a == 0x8000_0000 && b == 0xFFFF_FFFF { 0x8000_0000 }   // overflow
            else { ((a as i32).wrapping_div(b as i32)) as u32 }
        }
        0x5 => if b == 0 { u32::MAX } else { a / b },                       // DIVU
        0x6 => {                                                             // REM (signed)
            if b == 0 { a }
        ...
        _   => return Err(OarsError::Runtime { pc, msg: format!("illegal M-ext {word:#010x}") }),
    };
    regs.write(rd, v);
